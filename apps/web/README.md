...
**Migração para Tailwind CSS + shadcn/ui (`ADR-0050`, `ENG-0156`-`0157`)**: supera a decisão de CSS puro de `ENG-0147` — motivada pelo pedido do CTO de usar o catálogo real do 21st.dev (distribuído via `npx shadcn@latest add`, incompatível com `style` inline sem retrofit). Fase 1 foi só infraestrutura (tokens mapeados via CSS vars, zero mudança visual). Fase 2 migrou os 9 primitivos compartilhados e as 22 telas de `style` inline para classes Tailwind — mesmos valores visuais, hover/focus via variantes nativas em vez de handlers JS. Fase 3 (`ENG-0158`, instalação real de componentes do 21st.dev — skeleton loaders, empty states com CTA, stat cards com tendência, scroll reveal, cards com glow/tilt) fica para a missão seguinte.
