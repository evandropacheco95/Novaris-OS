...
**Migração para Tailwind CSS + shadcn/ui (`ADR-0050`, `ENG-0156`, Fase 1)**: supera a decisão de CSS puro de `ENG-0147` — motivada pelo pedido do CTO de usar o catálogo real do 21st.dev (distribuído via `npx shadcn@latest add`, incompatível com `style` inline sem retrofit). Fase 1 é só infraestrutura: `tailwind.config.ts` mapeia cor/fonte/raio/sombra para as mesmas CSS custom properties `--nov-*` já existentes em `globals.css` (nenhum token recriado); `components.json`/`lib/utils.ts` (`cn()`) criados manualmente. Nenhum componente/tela migrado ainda — Fases 2 (`ENG-0157`, migração mecânica de `style` inline para classes Tailwind) e 3 (`ENG-0158`, instalação real de componentes do 21st.dev) ficam para missões seguintes.
