...
| [ADR-0048](ADR-0048-asset-filerecord-campaign-association.md) | `Asset` — reaproveita `FileRecord` (Kernel, `@novaris/files`) já implementado, modelado como Internal Entity de `Campaign` referenciando-o por id; resolve a posse que `ADR-0033` deixou em aberto (Marketing vs. transversal) | Aceito |
| [ADR-0049](ADR-0049-widget-visualization-types.md) | `Widget` — 4 tipos de visualização (`kpi`/`list`/`donut`/`bar`), `metricKey` como string opaca interpretada só pelo Frontend (mantém Analytics desacoplado de outros Business Domains); desbloqueia o que `ADR-0034` tinha deixado explicitamente pendente | Aceito |
| [ADR-0050](ADR-0050-tailwind-shadcn-migration.md) | Migração de `apps/web` para Tailwind CSS + shadcn/ui, tokens `--nov-*` preservados via CSS vars (nunca recriados) — supera a decisão de CSS puro de `ENG-0147`, para viabilizar o uso real do catálogo do 21st.dev pedido pelo CTO. Executada em 3 fases (`ENG-0156`-`0158`) | Aceito |
